...
cleos -u $DSP_ENDPOINT push action dappservices selectpkg "[\"$LOCAL_TEST_ACCOUNT\",\"$PROVIDER\",\"$SERVICE\",\"$PACKAGE_ID1\"]" -p $LOCAL_TEST_ACCOUNT@active
...
cleos -u $DSP_ENDPOINT push action dappservices selectpkg "[\"$LOCAL_TEST_ACCOUNT\",\"$PROVIDER\",\"$SERVICE\",\"$PACKAGE_ID2\"]" -p $LOCAL_TEST_ACCOUNT@active
...
# init chainid
export CHAIN_ID=cf057bbfb72640471fd910bcb67639c22df9f92470936cddc1ade0e2f2e7dc4f
cleos -u $DSP_ENDPOINT push action $LOCAL_TEST_ACCOUNT xvinit "[\"$CHAIN_ID\"]" -p $LOCAL_TEST_ACCOUNT